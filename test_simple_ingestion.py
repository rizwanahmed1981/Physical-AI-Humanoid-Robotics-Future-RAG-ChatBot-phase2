#!/usr/bin/env python3
"""
Simplified test of the ingest_docs.py script to verify functionality.
"""
...
# Constants - using only one small file for testing
...
async def test_ingestion():
    """Test ingestion with a single small file."""
    try:
        ...
        # Find just one MDX file for testing
        search_pattern = os.path.join(DOCS_PATH, "chapter2.mdx")  # Use a specific small file
        mdx_files = glob.glob(search_pattern)

        if not mdx_files:
            print("No test file found, trying chapter1.md")
            search_pattern = os.path.join(DOCS_PATH, "chapter1.md")
            mdx_files = glob.glob(search_pattern)
        ...
        print(f"Found {len(mdx_files)} file(s) to test with")

        # Test with the first file
        file_path = mdx_files[0]
        ...
        # Take only first 2 chunks for testing to avoid rate limits
        chunks = chunks[:2]
        print(f"Processing first {len(chunks)} chunks for testing")
        ...
        print(f"Error in ingestion test: {str(e)}")
        import traceback
        traceback.print_exc()
