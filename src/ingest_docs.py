#!/usr/bin/env python3
"""
Document ingestion script for the Physical AI Humanoid Robotics project.
This script processes documentation files, generates embeddings using Cohere,
and stores them in Qdrant vector database and Neon PostgreSQL database.
"""
...
try:
    # Import the services using relative paths
    ...
    logger.error("Make sure you're running this script from the correct directory and dependencies are installed.")
    sys.exit(1)

# Constants
DOCS_PATH = os.path.join(os.path.dirname(__file__), '..', 'docs', 'chapters')
...
async def ingest_document(file_path: str, cohere_service: CohereEmbeddingService,
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Successfully stored {len(embeddings)} vectors in Qdrant for {file_path}")
        except Exception as e:
            logger.error(f"Error storing vectors in Qdrant for {file_path}: {str(e)}")
            return
        ...
        try:
            ...
            logger.info(f"Successfully stored metadata for {len(neon_metadata_list)} chunks in Neon for {file_path}")
        except Exception as e:
            logger.error(f"Error storing metadata in Neon for {file_path}: {str(e)}")
            return
